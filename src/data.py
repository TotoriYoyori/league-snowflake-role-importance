...
def load_diff_interval_by_match(
    ...
    try:
        ...
        diff_query = query.DiffIntervalByMatch(
            minute=minute,
            team=team,
            min_game_duration=min_game_duration,
        )
...
def get_eval_bundle(
    ...
    """Keyed on (minute, team, min_game_duration) — test_size is a fixed default
    owned by prep.split_train_test; random_state and win_loss_threshold are
    fixed constants in Settings (neither is user-adjustable per current scope)."""
    scaled_df, _ = get_pivoted_data(settings, minute, team, min_game_duration)
    error = load_error(scaled_df)
    if error is not None:
        return {"error": error}
    ...
    metrics = evaluation.evaluate_on_test(
        model, test_df, feature_cols, target_col,
        threshold=settings.win_loss_threshold,
    )
    return {
        "train_df": train_df,
        "test_df": test_df,
        ...
        "preview_df": evaluation.preview_predictions(
            test_df, metrics["y_proba"], target_col,
            threshold=settings.win_loss_threshold,
        ),
    }
